...
import ResumeUploader from '@/components/features/ResumeUploader';
import { ResumeData } from '@/types';
import { Briefcase, GraduationCap, Code, Github, Linkedin, Mail, ExternalLink } from 'lucide-react';
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-8 border border-gray-100">
          <div className="flex flex-col md:flex-row justify-between items-start gap-4">
            <div>
              ...
              <div className="flex flex-wrap gap-4 mt-3 text-gray-600">
                ...
                {resumeData.contact_info.linkedin && (
                  <a 
                    href={ensureUrl(resumeData.contact_info.linkedin)} 
                    target="_blank" 
                ...
                {resumeData.contact_info.github && (
                  <a 
                    href={ensureUrl(resumeData.contact_info.github)} 
                    target="_blank" 
                    rel="noopener noreferrer"
            ...
            <div className="bg-blue-50 px-4 py-2 rounded-lg text-blue-700 font-semibold whitespace-nowrap">
              {resumeData.years_of_experience} Years Exp
            </div>
          </div>
          ...
          <div className="md:col-span-2 space-y-6">
              ...
              <div className="space-y-8">
                {resumeData.experience.map((exp, i) => (
                  <div key={i} className="relative pl-6 border-l-2 border-gray-200">
                    ...
                    <ul className="list-disc list-inside space-y-1 text-gray-700 text-sm marker:text-gray-400">
                      {exp.description.map((point, j) => (
                        <li key={j}>{point}</li>
                      ))}
                    </ul>
            ...
            {resumeData.projects.length > 0 && (
              <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {resumeData.projects.map((proj, i) => (
                    <div key={i} className="group p-4 border rounded-lg hover:border-blue-300 hover:shadow-md transition-all">
                      <div className="flex justify-between items-start">
                        <div className="font-bold text-gray-800 group-hover:text-blue-600 transition-colors">
                            {proj.name}
                        </div>
                        {proj.link && (
                            <a href={ensureUrl(proj.link)} target="_blank" className="text-gray-400 hover:text-blue-600">
        ...
        <button 
          onClick={() => setResumeData(null)}
          className="fixed bottom-8 right-8 bg-gray-900 text-white px-6 py-3 rounded-full shadow-lg hover:scale-105 hover:bg-black transition-all font-medium flex items-center gap-2"
        >
